...
module.exports = function () {
    ...
    eventEmitter.on('next', function () {
        if (tasks.length > 0 && runnerIndex <= wokers) {
    ...
    eventEmitter.on('initialize', function () {
        ...
        var options = cucumberProgram.opts();
        log('options: ', options);
        log('args: ', cucumberProgram.args);
        tasks = Tasks.getTasks(cucumberProgram.args, options);
        if (options.wokers) {
            wokers = parseInt(options.wokers);
        }
        eventEmitter.emit('run');
    });
